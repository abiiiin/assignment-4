...
pub mod pallet {
	...
	const HTTP_REMOTE_REQUEST_N: &str = "https://api.coincap.io/v2/assets/polkadot";
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(10000)]
		pub fn submit_price_signed(origin: OriginFor<T>, integ: u64, decim: u128) -> DispatchResult {
			let _who = ensure_signed(origin)?;
			log::info!("submit_price_signed: ({}, {})", integ, decim);

			let mut denominator: u128 = 1;
			loop {
				if denominator > decim {
					break;
				}
				denominator *= 10;
			}
			let deci: Permill = Permill::from_rational(decim, denominator);

			Self::append_or_replace_price(integ, deci);
	...
	impl<T: Config> Pallet<T> {
		...
		fn append_or_replace_price(integ: u64, decim: Permill) {
			Prices::<T>::mutate(|prices| {
				if prices.len() == NUM_VEC_LEN {
					let _ = prices.pop_front();
				}
				prices.push_back((integ, decim));
				log::info!("Prices vector: {:?}", integ);
			});
		}
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			let dot_price = StorageValueRef::persistent(b"offchain-demo::dp-info");

			if let Ok(Some(dp_info)) = dot_price.get::<DataWraper>() {
				// dp-info has already been fetched. Return early.
				log::info!("cached dp-info: {:?}", dp_info);
				return Ok(());
			}
			...
			if let Ok(_guard) = lock.try_lock() {
				match Self::fetch_n_parse1() {
					Ok(dp_info) => {
						dot_price.set(&dp_info);
						//dp_info
						//Self::offchain_signed_tx1(dp_info.data.priceUsd.);
						log::info!("dp-info: {:?}", dp_info.data.priceUsd.len());
						let mut v1 = dp_info.data.priceUsd;
						let v2 = v1.split_off(3);
						v1.pop();
						let u1: u64 = str::from_utf8(&v1).unwrap().parse::<u64>().unwrap();
						let u2: u128 = str::from_utf8(&v2).unwrap().parse::<u128>().unwrap();
						log::info!("dsssssssss: {:?}", u1);
						log::info!("rrrrrrrrrr: {:?}", u2);
						Self::offchain_signed_tx1(u1, u2).expect("failed to call chain");
					}
		...
		fn fetch_n_parse1() -> Result<DataWraper, Error<T>> {
			let resp_bytes = Self::fetch_from_remote1().map_err(|e| {
				log::error!("fetch_from_remote error: {:?}", e);
				<Error<T>>::HttpFetchingError
			})?;
			...
			// Deserializing JSON to struct, thanks to `serde` and `serde_derive`
			let dp_info: DataWraper =
			serde_json::from_str(&resp_str).map_err(|_| <Error<T>>::TestError)?;
			Ok(dp_info)
		}
		...
		fn fetch_from_remote1() -> Result<Vec<u8>, Error<T>> {
			log::info!("sending request to: {}", HTTP_REMOTE_REQUEST_N);

			// Initiate an external HTTP GET request. This is using high-level wrappers from `sp_runtime`.
			let request = rt_offchain::http::Request::get(HTTP_REMOTE_REQUEST_N);
		...
		fn offchain_signed_tx1(integ: u64, decim: u128) -> Result<(), Error<T>> {
			// We retrieve a signer and check if it is valid.
			//   Since this pallet only has one key in the keystore. We use `any_account()1 to
			//   retrieve it. If there are multiple keys and we want to pinpoint it, `with_filter()` can be chained,
			let signer = Signer::<T, T::AuthorityId>::any_account();
			...
			let result = signer.send_signed_transaction(|_acct|
				// This is the on-chain function
				Call::submit_price_signed(integ, decim)
				);
